...
export function resampleTrajectory<T extends { lat: number; lng: number; alt: number }>(
    ...
    for (let i = 0; i < targetCount; i++) {
        ...
        const from = points[segIdx] as T & { speed?: number };
        const to = points[segIdx + 1] as T & { speed?: number };
        if ('speed' in from && 'speed' in to) {
            (interpolated as T & { speed?: number }).speed = from.speed! + (to.speed! - from.speed!) * t;
        }
