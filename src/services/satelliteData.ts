...
export async function fetchSatelliteData(): Promise<{
  ...
  if (cached) {
    try {
      ...
      if (Date.now() - timestamp < CACHE_TIMEOUT && data.satellites.length > 0) {
        ...
        } catch { /* stale cache still usable without a binary refresh */ }
        return data;
      }
    } catch {
  ...
  try {
    ...
    const [orbitalRes, listRes, satNogsRes] = await Promise.all([
    ...
    const listIndexData: ListIndexRecord[] = listRes;
    ...
    const listIndexMap = new Map<number, ListIndexRecord>();
    ...
    for (let i = 0; i < orbitalView.length; i += ELEMENT_COUNT) {
      ...
      const indexRecord: Partial<ListIndexRecord> = listIndexMap.get(noradId) || {};
    ...
    if (result.totalSatellites > 0) {
      try {
        const toCache: Omit<typeof result, 'orbitalBuffer'> & { orbitalBuffer?: Float32Array } = { ...result };
        toCache.orbitalBuffer = undefined; // Do not stringify Float32Array into LocalStorage
