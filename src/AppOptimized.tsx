...
import { fetchSatelliteData, fetchSatelliteDetail } from './services/satelliteData';
...
function AppOptimized() {
  ...
  const handleSelectSatellite = useCallback(async (sat: any, satList: any[]) => {
      ...
      {isMobile ? (
        ...
        <>
          ...
          {(satelliteWithLivePosition || showInfo) && (
            <div className="absolute top-0 right-0 h-full w-[360px] z-50 bg-zinc-900/90 border-l border-white/10 shadow-2xl overflow-y-auto">
              {showInfo ? (
                <InfoPage onClose={() => setShowInfo(false)} />
              ) : (
                <SatelliteDetailView
