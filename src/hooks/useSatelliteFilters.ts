    ...
    const filteredSatellites = useMemo(() => {
        ...
        if (activeOrbits.size > 0) {
            result = result.filter(sat => {
                const spacetrack = sat.masterData?.spacetrack as { ECCENTRICITY?: number } | undefined;
                const gcat = sat.masterData?.gcat as { OpOrbit?: string } | undefined;
                const regime = getOrbitRegime({
                    apoapsis: sat.apoapsis,
                    eccentricity: (sat.omm?.ECCENTRICITY as number | undefined) || spacetrack?.ECCENTRICITY,
                    gcatOpOrbit: gcat?.OpOrbit
                });
