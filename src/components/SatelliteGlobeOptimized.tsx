...
import type { PositionBufferRef } from '../AppOptimized';
...
export const SatelliteGlobeOptimized: React.FC<SatelliteGlobeOptimizedProps> = React.memo(({
  ...
  useEffect(() => {
    ...
    const getSatColor = (type: string | undefined): [number, number, number] => {
      switch (type) {
    ...
    const getPickedId = (event: MouseEvent) => {
      ...
      pickingGeo.copy(sphereGeo as any);
