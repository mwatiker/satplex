...
// Deeply nested external JSON of unknown/varying shape (UCS, OSCAR, GCAT, Space-Track,
// Gunter's, Wikipedia — each an independent upstream source with no shared schema).
// eslint-disable-next-line @typescript-eslint/no-explicit-any
type ExternalJson = any;
 ...
 */
const GroundTrackLoading = ({ isNarrow = false }: { isNarrow?: boolean }) => {
  ...
  return (
    <div className={`relative w-full ${isNarrow ? 'h-[120px]' : 'h-[180px]'} bg-black/40 rounded-lg border border-white/5 overflow-hidden mb-3 flex flex-col items-center justify-center gap-3`}>
      <svg
        ref={svgRef}
...
interface Satellite {
  ...
  ucs: ExternalJson;
  oscar: ExternalJson;
  satnogs: ExternalJson;
  gcat: ExternalJson;
  spacetrack: ExternalJson;
  gunters: ExternalJson;
  wiki: ExternalJson;
}
...
const isGeostationary = (spacetrack: ExternalJson, gcat: ExternalJson): boolean => {
  if (gcat?.OpOrbit) {
...
const GroundTrackMap = React.memo(({
  ...
  return (
    <div className={`relative w-full ${isNarrow ? 'h-[120px]' : 'h-[180px]'} bg-black/30 rounded-md border border-white/[0.06] overflow-hidden mb-3`}>
      <ComposableMap
...
const MetricCard = ({ label, value, unit, isNarrow = false }: { label: string; value: string | number; unit?: string; isNarrow?: boolean }) => (
  <div className="py-1">
    <div className="text-[9px] uppercase tracking-widest text-zinc-600 mb-1">{label}</div>
    <div className={`${isNarrow ? 'text-xs' : 'text-sm'} font-mono text-zinc-100 leading-none`}>{value}</div>
...
export default function SatelliteDetailView(props: {
  ...
  const { isMobile, isNarrow, ...rest } = props;
  ...
  return (
    <div className="w-full text-white">
      <SatelliteDetailHeader {...rest} isNarrow={isNarrow} />
      <div className="mt-4">
        <SatelliteDetailBody {...rest} isNarrow={isNarrow} />
      </div>
    </div>
  );
}
  ...
  return (
    <div className="px-4 pt-4 pb-2 space-y-2 text-white">
      <div className="flex justify-between items-start gap-4">
        <h1 className={`${isNarrow ? 'text-xl' : 'text-2xl'} font-black`}>{name}</h1>
        {onClose && (
          <button
            onClick={(e) => {
              e.stopPropagation();
...
export function SatelliteDetailBody({
  ...
  const identityFields = useMemo<[string, string][]>(() => {
    ...
    ] as [string, ExternalJson][]).filter(([, v]) => v && String(v).trim() !== '-' && String(v).toLowerCase() !== 'unknown') as [string, string][];
  ...
  const externalLinks = useMemo<[string, string][]>(() => {
    ...
    ] as [string, ExternalJson][]).filter(([, v]) => !!v && String(v).toLowerCase() !== 'unknown') as [string, string][];
  }, [data]);
  ...
  const trackContent = (() => {
          ...
          {isNarrow ? (
            <div className="flex flex-col items-center gap-1 px-1">
              <div className="flex items-center gap-2">
                <span className="text-[10px] text-zinc-500 font-mono">{currentPosition}</span>
              </div>
              {!isGeo && (
    ...
    return <GroundTrackLoading isNarrow={isNarrow} />;
  })();
  ...
  return (
    <div className="w-full space-y-6 p-4 pt-0 text-white">
      ...
      <div className={`${isNarrow ? 'grid grid-cols-2 gap-x-2 px-4' : 'flex justify-evenly'} border-t border-b border-white/[0.06] py-4`}>
        <MetricCard
          label="Altitude"
